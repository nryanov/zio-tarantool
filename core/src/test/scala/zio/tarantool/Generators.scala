...
object Generators {
  def nonEmptyString(maxLen: Int): Gen[String] = Gen.alphaStr.suchThat(str => str.nonEmpty && str.length <= maxLen)

  def bool(): Gen[Boolean] = Gen.oneOf(true, false)

  def listOf[A](maxSize: Int, gen: Gen[A]): Gen[List[A]] = Gen.listOfN(maxSize, gen)

  def nonEmptyListOf[A](maxSize: Int, gen: Gen[A]): Gen[List[A]] = listOf(maxSize, gen).suchThat(_.nonEmpty)

  def mapOf[A, B](maxSize: Int, key: Gen[A], value: Gen[B]): Gen[Map[A, B]] = Gen.mapOfN(maxSize, key.flatMap(k => value.map(v => (k, v))))
