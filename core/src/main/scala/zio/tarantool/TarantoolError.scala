...
sealed abstract class TarantoolError(message: String, cause: Option[Throwable])
...
object TarantoolError {
  ...
  final case class InternalError(cause: Throwable)
      extends TarantoolError(cause.getLocalizedMessage, Some(cause))
  ...
  final case class AuthError(message: String, code: ResponseCode)
      extends TarantoolError(s"$message. Code: $code", None)
  ...
  final case class IndexNotFound(space: String, index: String)
      extends TarantoolError(s"$space:$index", None)
  ...
  final case class UnknownResponseCode(mp: MessagePackPacket)
      extends TarantoolError("Unknown response code", None)
  ...
  final case class DeclinedOperation(syncId: Long, code: RequestCode)
      extends TarantoolError(s"$code -- $syncId", None)
