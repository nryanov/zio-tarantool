...
object MessagePackPacketCodec extends Codec[MessagePackPacket] {
  ...
  private val mpMapCodecWiden: Codec[MessagePack] = mpMapCodec.widen(_.asInstanceOf[MessagePack], {
  ...
  private val codec: Codec[MessagePackPacket] =
    (mpMapCodecWiden :: mpMapCodecWiden).xmap[MessagePackPacket](
      msg => MessagePackPacket(packetMapEncoder.decodeUnsafe(msg.head), packetMapEncoder.decodeUnsafe(msg.tail.head)),
      value => packetMapEncoder.encodeUnsafe(value.header) :: packetMapEncoder.encodeUnsafe(value.body) :: HNil
    )
  ...
  override def decode(bits: BitVector): Attempt[DecodeResult[MessagePackPacket]] = codec.decode(bits)
}
